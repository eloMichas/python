...
print("""
    Witaj w grze tekstowej przygotowanej specjalnie na potrzebe stworzenia odpowiedniego portfolio!
        Aby grać wystarczy tylko klawiatura (a w szczegolnośi 3 przyciski :/)
""")
print()
...
while not exit:
    try:
        while True:
            answer=str(input(f'A więc {name} aby rozpocząć grę wciśnij "W" : '))
            licznik+=1
        ...
        time.sleep(1)
print('A wiec wyobraz sobie ze jestes w lesie, las jest ogromny! \n A ty stoisz sam/sama pośród drzew...')
...
if total < 3:
    ...
    print(f'prawdziwy as z ciebie {name}...')
    time.sleep(1)
    print(f'twój wynik to {total}')
    time.sleep(0.5)
